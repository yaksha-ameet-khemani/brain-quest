...
const level3Templates: TemplateEntry[] = [
  {
    key: "exponent",
    run: () => {
 ...
 * (same skill, new numbers) rather than the exact same question, which
 * doesn't even exist as a fixed thing to repeat. See buildCheckupQuestions
 * in lib/buildRound.ts. */
 ...
 * generateMathQuestionWithKey tagged it with) - new random numbers, same
 * underlying skill. Falls back to a random template if the key is stale
 * (e.g. templates were renamed/removed since it was recorded). */
...
export function generateMathQuestion(level: Level): GeneratedQuestion {
  return generateMathQuestionWithKey(level);
}

export function generateMathQuestions(level: Level, count: number): GeneratedQuestionWithKey[] {
  const out: GeneratedQuestionWithKey[] = [];
  const seenPrompts = new Set<string>();
  let guard = 0;
  while (out.length < count && guard < count * 10) {
    guard++;
    const q = generateMathQuestionWithKey(level);
